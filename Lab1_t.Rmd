...
The following R code reads in the data set from the twins study, and stores it in a data frame called twins.  Think of a data frame as R's way of representing a spreadsheet - it's the most common way of storing a data set in R. The original spreadsheet is stored within the Sleuth3 package, and will load automatically when you run `library(Sleuth3)`.  We give the data frame a more descriptive name so that we can work with it later.  Here I have called it `twins`.  The arrow pointing to the left `<-` says to store `case0202` in the object named on the left side.

Run the code below now to read in the data set. No need to modify this code.

```{r}
twins <- case0202 ## case0202 loads with Sleuth3 package
twins <- twins %>% mutate(
